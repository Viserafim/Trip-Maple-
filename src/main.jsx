...
const initialDays = [
  ...
  {date:'18/09/2026',dow:'Sexta-feira',title:"Canada's Wonderland",city:'Vaughan',icon:'🎢',notes:"Dia inteiro no parque. Opção alternativa: Niagara Falls + Niagara-on-the-Lake.",activities:[{time:'08:00',title:'Saída de Barrie',place:'Barrie → Vaughan',duration:'~1h',desc:'Deslocamento.'},{time:'09:15',title:"Canada's Wonderland",place:'Vaughan',duration:'dia inteiro',desc:'Parque, atrações e áreas temáticas.'},{time:'13:00',title:'Almoço',place:'Dentro do parque',duration:'1h',desc:'Almoço no parque.'}]},
  {date:'19/09/2026',dow:'Sábado',title:'Barrie + Jogo do Barrie Colts às 18h30',city:'Barrie',icon:'🏒',notes:'Dia em Barrie com jogo do Barrie Colts às 18h30.',activities:[{time:'Dia',title:'Barrie',place:'Barrie',duration:'flexível',desc:'Dia livre em Barrie antes do jogo.'},{time:'18:30',title:'Barrie Colts',place:'Barrie',duration:'~3h',desc:'Jogo do Barrie Colts às 18h30.'}]},
...
function App(){
  ...
  function updateDay(updatedDay){
    ...
    if(updatedDay.date !== editingDay.date && days.some(d=>d.date===updatedDay.date)){
      alert('Essa data já possui um dia no roteiro. Escolha outra data.');
      return;
    }
    setDays(ds => ds.map(d=>d.date===editingDay.date ? updatedDay : d).sort((a,b)=>parseDate(a.date)-parseDate(b.date)));
  ...
  return (
    <main className="detail">
      ...
      <div className="detailActions">
        <button className="secondary" onClick={onEditDay}>
          <Pencil size={18}/>
          Editar dia
        </button>
